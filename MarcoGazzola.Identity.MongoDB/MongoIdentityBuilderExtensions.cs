...
namespace MarcoGazzola.Identity.MongoDB
{
    public static class MongoIdentityBuilderExtensions
    {
            ...
            where TUser : IdentityUser
        {
            ...
            builder.Services.AddSingleton<IUserStore<TUser>>(p => (IUserStore<TUser>)new UserStore<TUser>(usersCollectionFactory(p)));
            builder.Services.AddSingleton<IRoleStore<TRole>>(p => (IRoleStore<TRole>)new RoleStore<TRole>(rolesCollectionFactory(p)));
            return builder;
        }
        ...
        public static IdentityBuilder AddIdentityWithMongoStoresUsingCustomTypes<TUser, TRole>(this IServiceCollection services, string connectionString)
            ...
            where TRole : IdentityRole
        {
            return services.AddIdentity<TUser, TRole>().RegisterMongoStores<TUser, TRole>(connectionString);
        }
    }
}